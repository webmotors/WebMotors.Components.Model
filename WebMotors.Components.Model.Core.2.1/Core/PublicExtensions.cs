...
namespace WebMotors.Components.Model.Core
{
	public static class PublicExtensions
	{
		...
		{
			return (_type.GetTypeInfo().IsGenericType && _type.GetGenericTypeDefinition().Equals(typeof(Nullable<>)));
		}
					...
					if (DBNull.Value != _value)
					{
						...
						else
						{
							if (IsNullableType(_type))
								return (T)Convert.ChangeType(_value, Nullable.GetUnderlyingType(_type));
