		...
		where TPk : IComparable<TPk>
	{
		...
		public dynamic getSQLSaveString(string connection, string procName = "Insert")
		{
			using (Database database = new Database(connection, false))
			{
				...
				var atributos = this.GetType().GetCustomAttributes<MapperTableAttribute>(false);
		...
		protected ValidationResult Delete(Database database, string procName = "Delete", bool ignoreValidation = false)
		{
			...
			var attributes = this.GetType().GetCustomAttributes<MapperTableAttribute>(false);
			...
			where T : BaseModel<TPk>
		{
			foreach (var p in getEntity.ListEntity)
			{
				...
				foreach (var attribute in atributeListRelation)
				{
					if (attribute.AutomaticGet)
					{
						Assembly objAssembly = Assembly.GetAssembly(p.PropertyType);
						IListEntity listEntity = (IListEntity)objAssembly.CreateInstance(p.PropertyType.FullName, false);
						listEntity.Set();
			...
			if (value != DBNull.Value)
			{
				if (!p.PropertyType.IsEnum)
				{
					...
					if (p.PropertyType.IsGenericType && p.PropertyType.GetGenericTypeDefinition().Equals(typeof(Nullable<>)))
					{
						...
						valorConvertido = Convert.ChangeType(value, nullableConverter.UnderlyingType);
					}
					else
		...
		private static GetEntity getEntityType(Type type, string procedure, string foreinKey = "")
		{
			...
			Assembly objAssembly = Assembly.GetAssembly(type);
			getEntity.Instance = objAssembly.CreateInstance(type.FullName, false);
		...
		{
			var atributos = instance.GetType().GetCustomAttributes<MapperTableAttribute>(false);
			foreach (var atributo in atributos)
			{
		...
		private static void getRelations(GetEntity getEntity, PropertyInfo p, object instance, bool left, ref int alias, string foreinKey = "", string procedure = "")
		{
			...
			foreach (var attribute in attributeRelation)
			{
				if (attribute.IsValid(procedure) && attribute.AutomaticGet && (!left || !attribute.InnerJoin) && (string.IsNullOrWhiteSpace(foreinKey) || attribute.ForeignKey != foreinKey))
				{
					...
					Assembly objAssembly2 = Assembly.GetAssembly(attribute.ClassRealtionship);
					noGetEntity.Instance = objAssembly2.CreateInstance(attribute.ClassRealtionship.FullName, false);
		...
		public void FillEntity(Dictionary<string, object> fields, string procedure = null)
		{
			...
			{
				object[] attributes = p.GetCustomAttributes(typeof(MapperFieldAttribute), false);
		...
		protected static T FillEntity<T>(DbDataReader dr, T instance = default(T), string procedure = null)
		{
			if (instance == null)
			{
				...
				instance = (T)objAssembly.CreateInstance(typeof(T).FullName, false);
			}
		...
		private static void SetFieldDataReader<T>(DbDataReader dr, T instance, Dictionary<string, string> fields, string procedure)
		{
			...
			var classAttr = instance.GetType().GetCustomAttributes<MapperTableAttribute>(false);
			...
			foreach (PropertyInfo p in properties)
			{
				...
				foreach (object attribute in attributes)
				{
					Assembly objAssembly2 = Assembly.GetAssembly(((MapperRelationshipAttribute)attribute).ClassRealtionship);
					var instancia2 = objAssembly2.CreateInstance(((MapperRelationshipAttribute)attribute).ClassRealtionship.FullName, false);
			...
			where T : BaseModel<TPk>
		{
			...
			var attributes = entidade.GetType().GetCustomAttributes<MapperFieldClassAttribute>(false);
			foreach (var attribute in attributes)
			{
				if (attribute.IsValid(procedure))
				{
			...
			var tableAttribute = entidade.GetType().GetCustomAttributes<MapperTableAttribute>(false);
