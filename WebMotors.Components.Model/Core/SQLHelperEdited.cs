...
namespace WebMotors.Components.Model.Core
{
	public class SQLHelperEdited
	{
		...
		#region [ DATA TABLE ]
		public static DataTable ExecuteDataTable(Database database, string pNameProcedure, ArrayList pParameters, CommandType pCommandType = CommandType.StoredProcedure)
		{
			using (DbCommand oSqlCommand = SQLHelperEdited.CreateCommand(database, pNameProcedure, pParameters, pCommandType))
			{
				return CreateDataTable(database, oSqlCommand);
			}
		}

		private static DataTable CreateDataTable(Database database, DbCommand command)
		{
